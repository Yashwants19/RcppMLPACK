...
#'   bucket.  Default value "6" (integer).
#' @param input_model Decision stump model to load (DSModel).
#' @param labels Labels for the training set. If not specified, the labels are
#'   assumed to be the last row of the training data (integer row).
#' @param test A dataset to calculate predictions for (numeric matrix).
#' @param training The dataset to train on (numeric matrix).
#' @param verbose Display informational messages and the full list of parameters and
#'   timers at the end of execution.  Default value "FALSE" (logical).
#'
#' @return A list with several components:
#' \item{output_model}{Output decision stump model to save (DSModel).}
#' \item{predictions}{The output matrix that will hold the predicted labels for the
#'   test set (integer row).}
#'
#' @details
